...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Im Doin Ur Mom RN', 0.2], //From 0% to 19%
		...
		['Go Outside', 0.9], //From 80% to 89%
		['Get a Life', 1], //From 90% to 99%
		['Literal Sweat', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case "monojack":
					snapCamFollowToPos(730, 450);
					startCountdownNoSound();
	...
	public function startCountdownNoSound():Void
		{
			if(startedCountdown) {
				callOnLuas('onStartCountdown', []);
				return;
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
				{
					...
					var introAssets:Map<String, Array<String>> = new Map<String, Array<String>>();
					introAssets.set('default', ['ready0', 'set0', 'go0']);
					introAssets.set('pixel', ['pixelUI/ready-pixel', 'pixelUI/set-pixel', 'pixelUI/date-pixel']);
